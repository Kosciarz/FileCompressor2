...
static void WriteCodesToFile(const std::string& file_path, const std::vector<int>& codes);

void CompressFile(const std::string& file_path)
{
	...
	for (const char& character : text_to_compress)
	{
		...
		{
			codes.push_back(dict.at(sequence));
	...
	WriteCodesToFile(file_path, codes);
}
...
void WriteCodesToFile(const std::string& file_path, const std::vector<int>& codes)
{
	std::fstream file;
